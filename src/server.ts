...
async function extractCobaltToken(page: Awaited<ReturnType<typeof getPage>>): Promise<string | null> {
...
async function fetchCharacterById(
  context: Awaited<ReturnType<typeof getSharedContext>>,
  ...
  try {
    ...
    if (resp.ok()) {
      ...
      return raw.data as DDBCharacter;
    }
  } catch {}
...
export function createServer(): McpServer {
  ...
  server.tool(
    "ddb_get_campaign_sheets",
    "Fetch the full character sheet for every character in a campaign. Intended for use when you are the DM.",
    {
    ...
    async ({ campaignId }) => {
      try {
        ...
        if (sheets.length) output.push(`\n---\n\n${sheets.join("\n\n---\n\n")}`);
        if (errors.length) output.push(`\n\n---\n\n**Fetch errors:**\n${errors.map((e) => `- ${e}`).join("\n")}`);
      ...
      try {
        ...
        const campaign = await page.evaluate(() => {
          const data: {
            ...
            description?: string;
            characters?: Array<{ character: string; id: string; level: string; player: string }>;
          } = {};
          ...
          const characters: Array<{ character: string; id: string; level: string; player: string }> = [];
          document.querySelectorAll("li.ddb-campaigns-character-card-wrapper").forEach((el) => {
            const charName =
              el
                .querySelector(
            ...
            const link = el.querySelector("a[href*='/characters/']") as HTMLAnchorElement | null;
            const idMatch = (link?.href ?? "").match(/\/characters\/(\d+)/);
            const id = idMatch?.[1] ?? "";
            if (charName) characters.push({ character: charName, id, level, player });
          });
        ...
        if (campaign.characters?.length) {
          ...
          for (const c of campaign.characters) {
            ...
            lines.push(
              `- **${c.character}**${idPart} — ${c.level}${c.player ? ` (Player: ${c.player})` : ""}`,
            );
          }
        }
